...
namespace ReminNote.Pet.Startup;
...
{
    public const string MutexName = @"Local\ReminNote.Pet.Demo";
    public const string PipeName = "ReminNote.Pet.Demo.Activation";
    public const string ActivationMessage = "activate";

    public static string GetPipeName(string profileScope)
    {
        var input = string.Concat("ReminNote.PetInstance.v1", '\0', profileScope);
        var digest = SHA256.HashData(Encoding.UTF8.GetBytes(input));
        var instanceHash = Convert.ToHexString(digest).ToLowerInvariant()[..32];
        return $"ReminNote.Pet.Activation.{profileScope}.{instanceHash}";
    }
}
