...
& 'C:\Program Files\dotnet\dotnet.exe' build 'D:\桌宠\dafeiyu\dafeiyu.csproj'

# 运行（Debug 输出）
& 'D:\桌宠\dafeiyu\bin\Debug\net10.0-windows\dafeiyu.exe'
```

也可以在 Visual Studio / Rider 打开 `dafeiyu.csproj` 直接运行。
...
- **位置持久化**：窗口位置与收纳态存到 `%LOCALAPPDATA%\dafeiyu\pet_state.json`（纯呈现状态）。

## 项目结构

```text
D:\桌宠\dafeiyu\
  dafeiyu.csproj      # net10.0-windows, UseWPF + UseWindowsForms(托盘), 零外部包
